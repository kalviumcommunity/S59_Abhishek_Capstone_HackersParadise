...
export default function Bounties() {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(
        `https://s59-abhishek-capstone-hackersparadise.onrender.com/api/bounties-delete/${id}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleUpdate = async (bounty) => {
    try {
      const response = await fetch(
        `https://s59-abhishek-capstone-hackersparadise.onrender.com/api/bounties-update/${bounty._id}`,
        {
          method: "PUT",
          headers: {
      ...
      setData((prevData) => {
        const index = prevData.findIndex((b) => b._id === updatedBounty._id);
        if (index !== -1) {
        ...
        <div className="flex flex-wrap w-full justify-center">
          {data.length != 0 ? (
            filteredData.map((bounty) => (
              <div
                key={bounty._id}
                className="h-full w-[20vw] bg-[#b25ffb] m-[2vw] rounded"
              >
                ...
                <div className="m-6">
                  ...
                  <button className="h-[4vh] bg-gray-200 p-[0.5vh] rounded text-gray-600 font-bold mt-2 mr-2">
                    Other Asset
                  </button>
                  <button className="h-[4vh] bg-gray-200 p-[0.5vh] rounded text-gray-600 font-bold mt-2 mr-2">
                    Executable
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
            <div className="bg-white p-4 rounded">
              ...
              <input
                ...
                onChange={(e) =>
                  setEditBounty({ ...editBounty, company: e.target.value })
                }
              />
              <input
                type="text"
                ...
                onChange={(e) =>
                  setEditBounty({ ...editBounty, reward: e.target.value })
                }
              />
              <div className="flex justify-around mt-2">
                <button
