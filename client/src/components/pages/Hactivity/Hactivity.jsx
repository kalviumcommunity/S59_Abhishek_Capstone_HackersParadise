...
export default function Hactivity() {
  ...
  useEffect(() => {
    fetch("https://s59-abhishek-capstone-hackersparadise.onrender.com/api/hactivity")
    ...
    <div className="bg-[#000746] h-full">
      ...
      {hactivities.length !== 0 ? (
        hactivities.map((hactivity) => (
          <div
            key={hactivity.id}
            className="bg-[#9f54ff] w-[80%] m-auto mb-[5vh] text-white flex justify-between rounded-xl"
          >
            <div className="p-6">
              <p className="text-[1.5rem] font-semibold">{hactivity.company}</p>
              <p>{hactivity.info}</p>
            </div>
            ...
            <div className="bg-gradient-to-r from-[#d48ff9] via-[#b25ffb] to-[#6300ff] w-[80%] m-auto mb-[5vh] text-white flex justify-between items-center p-4 rounded-xl">
              ...
              />
              <button
                onClick={() => handleComment(hactivity.id)}
